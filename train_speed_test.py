"""
Speed Test Training Script - bypasses slow components to test training speed
"""
...
def main():
    ...
    # Setup dataset - USE FAST DATASET
    logger.info("Setting up FAST dataset...")
    train_dataset = FastCOCOScribbleDataset(
        config=config.data,
        tokenizer=tokenizer,
        split="train",
    ...
    logger.info("Starting SPEED TEST training...")
    global_step = 0
    epoch = 0
    ...
    while global_step < config.training.max_train_steps:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            recent_avg = sum(step_times[-5:]) / min(5, len(step_times))  # Last 5 steps
            ...
            if accelerator.is_main_process:
                ...
                progress_bar.set_postfix({
                    ...
                    'recent_avg': f'{recent_avg:.1f}s',
                    'memory': f'{memory_gb:.1f}GB'
                })
                progress_bar.update(1)
                
                # Log detailed timing every few steps
                if global_step % 5 == 0:
                    ...
                    print(f"\n⏱️  Step {global_step}/{config.training.max_train_steps}")
                    print(f"    Current step: {step_time:.2f}s")
                    print(f"    Average step: {avg_step_time:.2f}s") 
    ...
    if accelerator.is_main_process:
        ...
        print(f"   Fastest step: {min(step_times):.2f}s")
